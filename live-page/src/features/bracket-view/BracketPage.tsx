...
export default function BracketPage() {
  ...
  const matches = useMemo(
    () => allMatches.filter((m) => m.eventId === eventId).sort((a, b) => a.round - b.round || a.position - b.position),
    [allMatches, eventId],
  );
  ...
  const roundGroups = useMemo(() => {
    ...
    for (const [, arr] of groups) arr.sort((a, b) => a.position - b.position);
    return groups;
  }, [matches]);
    ...
    <div className="max-w-full mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mt-2 mb-4">
        ...
        <div className="flex items-center gap-3">
          {drawSize > 0 && <span className="text-xs text-gray-400">ドローサイズ: {drawSize}</span>}
          <LastUpdated />
        </div>
      </div>
                      ...
                      return (
                        <div
                          ...
                          className={`rounded-lg border overflow-hidden text-xs ${
                            isPlaying ? 'match-playing border-orange-500/50'
                              : isFinished ? 'border-white/10 opacity-80' : 'border-white/10'
                          }`}
                        >
                          <div className={`flex items-center justify-between px-2 py-1.5 ${
                            isFinished && p1Won ? 'bg-emerald-500/10'
                              : isFinished && !p1Won ? 'bg-white/3 text-gray-500' : 'bg-white/5'
                          }`}>
                            <span className={`truncate block flex-1 ${p1Won ? 'font-semibold text-emerald-300' : ''}`}>
                          ...
                          <div className="border-t border-white/10 flex items-center justify-between px-2">
                            {isPlaying && <span className="text-[9px] text-orange-400 py-0.5">LIVE</span>}
                            {isFinished && match.score && (
                              <span className="text-[9px] text-gray-500 py-0.5 mx-auto">{match.score}</span>
                            )}
                          </div>
                          <div className={`flex items-center justify-between px-2 py-1.5 ${
                            isFinished && p2Won ? 'bg-emerald-500/10'
                              : isFinished && !p2Won ? 'bg-white/3 text-gray-500' : 'bg-white/5'
                          }`}>
                            <span className={`truncate block flex-1 ${p2Won ? 'font-semibold text-emerald-300' : ''}`}>
