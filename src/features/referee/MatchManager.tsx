...
export default function MatchManager({ readOnly = false }: { readOnly?: boolean } = {}) {
  ...
  // --- テスト用: 全試合を 8-4 / 6-4 で終了させる（動作確認用・運営画面のみ） ---
  const [testScoreRunning, setTestScoreRunning] = useState(false);
  const handleFillTestScores = useCallback(async () => {
    if (!currentTournamentId) return;
    const ok = confirm(
      'テスト用に、全種目の全試合へ 8-4 / 6-4 のスコアを入れて決勝まで終了させます。\n'
      + '入力済みのスコアは上書きされます。よろしいですか？',
    );
    if (!ok) return;
    ...
    try {
      ...
      alert(
        res.filledCount === 0
          ...
          : `${res.eventCount}種目・${res.filledCount}試合にテストスコアを入力しました。`,
      );
    } catch (err) {
      console.error('テストスコア入力に失敗:', err);
      alert(`テストスコアの入力に失敗しました: ${(err as Error).message}`);
    } finally {
      ...
      <div className="lg:w-[280px] shrink-0 order-1 lg:order-1 mb-3 lg:mb-0 sticky top-0 z-20 lg:self-start bg-bg-main pb-1">
        ...
        <div className={`transition-all duration-300 overflow-hidden ${controlsOpen ? 'max-h-[600px] opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
          <div className="bg-white p-4 rounded-xl shadow-sm border border-border-main space-y-3">
            ...
            {/* テスト用: 全試合を 8-4 / 6-4 で終了（動作確認用） */}
            <button
              onClick={handleFillTestScores}
              disabled={testScoreRunning}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold text-gray-600 bg-white border border-dashed border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50 transition-all"
            >
              <Check className="w-3.5 h-3.5" />
              {testScoreRunning ? '入力中...' : 'テスト: 全試合を8-4/6-4で終了'}
            </button>
            ...
            <div className="space-y-2">
                ...
                {(() => {
                  ...
                  return shown.map((m) => {
                    ...
                    if (isPlaying) {
                      statusDisplay = { text: '試合中', color: 'bg-primary-100 text-primary-700' };
                    } else if (isFinished) {
                      statusDisplay = st;
                    } else if (!hasPlayers) {
                      ...
                      <React.Fragment key={matchKey(m)}>
                        ...
                        >
                          ...
                          <div className="relative flex items-center gap-2 mt-1.5 pl-1 min-h-[36px]">
                            ...
                            <div className="flex-1" />
                            {elapsedLabel && (
                              <span className="absolute left-1/2 -translate-x-1/2 inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-primary-100 text-primary-700 text-[11px] font-bold font-mono">
                            ...
                            <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                              ...
                              {evt && (
                                <button
                                  ...
                                  className="p-1.5 text-primary-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg border border-primary-200 transition-all"
                                  title="試合ルール"
                                >
                                  ...
                                  <tr key={matchKey(m)} className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                                    ...
                                    <td className="py-2.5 px-2">
                                      <div className="flex items-center gap-1">
                                        ...
                                        <span className={`whitespace-nowrap text-sm ${autoWinner === 1 ? 'font-bold text-primary-800' : autoWinner === 2 ? 'text-gray-400' : 'font-medium'}`}>
                                          {m.player1Name}
                                        </span>
                                      </div>
                                    </td>
                                    ...
                                    <td className="py-2.5 px-2">
                                      <div className="flex items-center gap-1">
                                        ...
                                        <span className={`whitespace-nowrap text-sm ${autoWinner === 2 ? 'font-bold text-primary-800' : autoWinner === 1 ? 'text-gray-400' : 'font-medium'}`}>
                                          {m.player2Name}
                                        </span>
                                      </div>
                                    </td>
                                    ...
                                    <td className="py-2.5 px-2 text-center">
                                      {autoWinner ? (
                                        <span className="text-[10px] text-primary-600 font-bold bg-primary-100 px-1.5 py-0.5 rounded-full">
