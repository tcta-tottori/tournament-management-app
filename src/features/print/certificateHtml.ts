...
import { certFontCssUrl, getCertFont } from './certificateFonts';
...
function styleHtml(layout: CertLayout): string {
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  body { font-family: ${font.stack}; color: #000; -webkit-font-smoothing: antialiased; }
  .page {
  ...
  .cert-category { font-size: ${layout.categorySize}pt; letter-spacing: ${layout.tracking}em; }
  .cert-rank { font-size: ${layout.rankSize}pt; letter-spacing: ${layout.tracking + 0.2}em; }
  .cert-affiliation { font-size: ${layout.affiliationSize}pt; letter-spacing: ${layout.tracking}em; }
  .cert-name { font-size: ${layout.nameSize}pt; letter-spacing: ${layout.tracking}em; }
  ...
  .cert-title {
    ...
    font-size: ${Math.round(layout.nameSize * 1.15)}pt; letter-spacing: 0.5em;
  }
  .cert-event {
...
export function buildCertificateHtml(entries: CertEntry[], layout: CertLayout): string {
  const font = getCertFont(layout.fontId);
  const fontUrl = certFontCssUrl(font);
