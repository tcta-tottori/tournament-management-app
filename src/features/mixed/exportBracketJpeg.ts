...
function rRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, _r: number, fill?: string, stroke?: string, sw = 1) {
...
export async function generateBracketDataUrl(
  ...
  rRect(ctx, PADDING_X, PADDING_Y - 2, cw, 30, 0, '#fff', '#222', 2);
  txt(ctx, catLabel, PADDING_X + cw / 2, PADDING_Y + 13, 16, { align: 'center', bold: true });
  ...
  const drawBracket = (
    ...
    // 縦線
    ln(ctx, jx, t1cy, jx, t2cy, hasW ? WIN_COLOR : LINE_COLOR, hasW ? WIN_W : LOSE_W);
    // 出力水平線
    ...
    if (m.status === 'finished' && m.score1 != null && m.score2 != null) {
      if (isLeft) {
        txt(ctx, String(m.score1), jx + 6, t1cy, SCORE_SIZE, { color: SCORE_COLOR, bold: true });
        txt(ctx, String(m.score2), jx + 6, t2cy, SCORE_SIZE, { color: SCORE_COLOR, bold: true });
      } else {
        txt(ctx, String(m.score1), jx - 6, t1cy, SCORE_SIZE, { align: 'right', color: SCORE_COLOR, bold: true });
        txt(ctx, String(m.score2), jx - 6, t2cy, SCORE_SIZE, { align: 'right', color: SCORE_COLOR, bold: true });
      }
    }
  };
  ...
  for (let i = 0; i < leftR1.length; i++) {
    ...
    if (bye1 || bye2) {
      ...
      const jx = slotR + gapX * 0.45;
      drawBracket(t1cy, t2cy, cy, slotR, jx, exitX, m, true);
  ...
  for (let i = 0; i < rightR1.length; i++) {
    ...
    if (bye1 || bye2) {
      ...
      const jx = slotL - gapX * 0.45;
      drawBracket(t1cy, t2cy, cy, slotL, jx, exitX, m, false);
      jp.set(m.matchId, { x: exitX, y: cy });
    }
  }
  ...
  for (let r = 2; r <= sideRounds; r++) {
    ...
    for (const m of rms) {
      ...
      if (isLeft) {
        const jx = baseX + gapX * 0.45;
        const exitX = baseX + gapX;
        drawBracket(upperY, lowerY, cy, baseX, jx, exitX, m, true);
        jp.set(m.matchId, { x: exitX, y: cy });
      } else {
        const jx = baseX - gapX * 0.45;
        const exitX = baseX - gapX;
        drawBracket(upperY, lowerY, cy, baseX, jx, exitX, m, false);
        jp.set(m.matchId, { x: exitX, y: cy });
      }
    }
  }
  ...
  if (maxRound >= 2) {
    ...
    if (finals.length > 0) {
      ...
      if (parents.length >= 2) {
        ...
        ln(ctx, jx, upperY, jx, lowerY, hasW ? WIN_COLOR : LINE_COLOR, hasW ? WIN_W : LOSE_W);

        if (fm.status === 'finished' && fm.score1 != null && fm.score2 != null) {
          txt(ctx, String(fm.score1), jx + 7, upperY, SCORE_SIZE + 1, { color: SCORE_COLOR, bold: true });
          txt(ctx, String(fm.score2), jx + 7, lowerY, SCORE_SIZE + 1, { color: SCORE_COLOR, bold: true });
        }

        if (fm.winnerId) {
          const w = allTeams.find(t => t.teamId === fm.winnerId);
          if (w) {
            const label = `${familyName(w.male.name)}・${familyName(w.female.name)}`;
            txt(ctx, label, jx, cy - 5, 12, { align: 'center', bold: true });
            if (fm.score1 != null && fm.score2 != null) {
              txt(ctx, `${fm.score1}−${fm.score2}`, jx, cy + 12, 11, { align: 'center', color: '#555' });
            }
          }
        }
      }
