...
export default function MixedBracketResultPreview({ opts, size = 'sm', label = '結果画像' }: Props) {
  ...
  return (
    <>
      ...
      {isOpen && createPortal(
        <div
          className="fixed inset-0 bg-gray-950/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4"
        ...
        >
          ...
          >
            ...
            <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50/50 text-[11px] text-gray-700 flex items-center justify-between gap-2 shrink-0">
              ...
              {dataUrl && (
                <button
                  onClick={handleDownload}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-600 text-white font-bold hover:bg-gray-700 transition-colors active:scale-95 shrink-0"
                >
                  <Download size={13} />
                  JPEGで保存
                </button>
