...
export default function MixedStandingsView() {
  ...
  const rankColors = [
    'from-primary-400 to-primary-500', // 1位
    'from-gray-300 to-gray-400',    // 2位
    'from-primary-300 to-primary-400', // 3位
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {leagues.map(league => {
          ...
          return (
            <div key={league.leagueId} className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all hover:shadow-md ${isComplete ? 'border-primary-200' : 'border-gray-200'}`}>
              {/* カードヘッダー */}
              <div className={`px-4 py-3 flex items-center justify-between ${isComplete ? 'bg-gradient-to-r from-primary-50 to-white' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2">
                  <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold text-white ${isComplete ? 'bg-gradient-to-br from-primary-500 to-primary-600' : 'bg-gray-400'}`}>
            ...
            {[
              ...
              return (
                <div key={rank} className="space-y-2">
                  <div className={`text-sm font-bold px-3 py-2 rounded-lg ${chip}`}>
