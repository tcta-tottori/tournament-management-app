...
const FEATURE_SECTIONS: FeatureSection[] = [
  ...
  },
  {
    ...
    keyFeatures: [
      ...
      '毛筆（行書・楷書）・手書き・明朝など複数の書体から選択',
      '市販の賞状用紙に重ねる「文字のみ印刷」と、枠・題字ごと刷る「枠つき」の切替',
      '用紙サイズ（A4/B4/B5・縦横）、縦書き、位置・文字サイズ・字間・行間の調整',
      '印刷と同じ見た目のプレビュー（印字範囲のガイド付き）',
      '書式と印刷リストは端末に保存され、次に開いたときも残る',
    ],
    operationSteps: [
      { step: 1, title: '賞状に入れる人を決める', description: '上の一覧から選ぶか、「手動で追加」で直接入力します。氏名・賞位・クラス名・所属はあとから直せます。' },
      { step: 2, title: '書体・レイアウトを合わせる', description: '「書式設定」を開いてフォントと用紙を選び、上下位置・左右位置・文字サイズを賞状用紙に合わせます。' },
    ...
    tips: [
      ...
      '「PCのフォント」はWindowsのHG行書体・HG正楷書体を使います（入っていない端末では游明朝になります）',
    ],
  },
  {
