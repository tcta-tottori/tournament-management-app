...
import { db, type Draw, type Entry, type Match } from '../../db/database';
import { useAppStore } from '../../stores/appStore';
import HeatRulePatternInput, { applyHeatToRules } from '../data/HeatRulePatternInput';
import { CheckSquare, UserCheck, UserPlus, Search, Eye, List, AlertCircle, ChevronDown, ChevronRight, ChevronUp, RotateCcw, Lock, Ban, Unlock, X, Check, Clock, Shuffle, ArrowDownToLine, ArrowUpToLine } from 'lucide-react';
import ProcessingModal, { type ProcessingStep } from '../../components/ui/ProcessingModal';
import ConfirmDialog from '../../components/ui/ConfirmDialog';
import { generateScheduleFromDraws, AUTO_GENERATED_SCHEDULE_LABEL, isAutoGeneratedSchedule } from '../schedule/generateSchedule';
import { repairShiftedDrawTimes } from '../schedule/repairDrawTimes';
import {
...
function NormalEntryRegistration() {
  ...
  // === あたり（対戦の組み合わせ）修正 ===
  // 取り込んだドロー表と実際のあたりが違うとき、この画面で枠を並べ替えて直す。
  // 保存するまではドラフトだけを書き換えるので、ブラケット表示で結果を確認できる。
  const [drawEdit, setDrawEdit] = useState<{ eventId: string; slots: Draw['slots'] } | null>(null);
  ...
  const buildSlotsForEvent = useCallback((eventId: string): CheckInSlot[] => {
    ...
    // あたり修正中の種目は、保存前のドラフトで表示する（ブラケットで結果を確認できる）
    const drawSlots = drawEdit?.eventId === eventId ? drawEdit.slots : draw?.slots;

    if (draw && drawSlots && drawSlots.length > 0) {
      return drawSlots
        .slice()
  ...
  }, [drawMap, allEntries, entryMap, playerMap, drawEdit]);
  ...
  const maybeRegenerateSchedule = useCallback(async () => {
    const { scheduleFileName, importedSchedule, scheduleConfig, currentTournamentId: tid } = useAppStore.getState();
    if (!tid) return;
    // 自動生成した時間割のみ再編成対象（取込済みの固定時間割は保持）
    if (importedSchedule.length === 0) return;
    if (!isAutoGeneratedSchedule(scheduleFileName)) return;

    // 既存の使用コート・生成条件（開始時刻・試合間隔）を引き継いで再生成する。
    // 条件を引き継がないと、確定のたびに時間割の刻みが変わってしまう。
    ...
    const courtNames = courts
      ...
      .map(c => c.name);
    const startTime = scheduleConfig.startTime || tournament?.drawStartTime || '09:00';

    const result = await generateScheduleFromDraws(tid, {
      courtNames: courtNames.length > 0 ? courtNames : undefined,
      startTime,
      matchDuration: scheduleConfig.matchDuration || undefined,
      // 保存済みの条件よりドロー表の記載時刻を優先する。
      // （前回ずれた条件で保存されていても、確定のたびにドロー表どおりへ戻す）
      preferDrawBase: true,
    });
  ...
  const recalculateGlobalMatchOrder = useCallback(async () => {
    ...
    // 消費済みの試合（時刻一致で先に取り出した分をスキップするため）
    const consumed = new Set<number>();

    /** ドロー表に記載された、その試合の開始時刻（無ければ null） */
    const drawTimeOf = (m: Match): string | null => {
      const draw = drawMap.get(m.eventId);
      if (!draw) return null;
      if (m.round === 1) return draw.matchTimes?.[2 * m.position - 1] || null;
      return draw.roundMatchTimes?.[`R${m.round}-${m.position}`] || null;
    };

    /**
     * キューから次に割り当てる試合を取り出す。
     * ドロー表に開始時刻が記載されている試合は、その時刻と一致する枠へ優先的に入れる。
     * （順番だけで割り当てると、9:40開始の試合が9:00枠に入るなど、ドロー表と
     *   ずれた対戦が初回に並んでしまう）
     */
    const takeFromQueue = (key: string, itemStartTime: string): Match | null => {
      const queue = matchQueues.get(key);
      if (!queue) return null;
      // 1) この枠の時刻と同じドロー記載時刻を持つ、未消費の試合（position順）
      if (itemStartTime) {
        for (const m of queue) {
          if (m.id == null || consumed.has(m.id)) continue;
          if (drawTimeOf(m) === itemStartTime) {
            consumed.add(m.id);
            return m;
          }
        }
      }
      // 2) 見つからなければ、ドロー記載時刻が無い試合を順番に
      for (const m of queue) {
        if (m.id == null || consumed.has(m.id)) continue;
        if (drawTimeOf(m)) continue;
        consumed.add(m.id);
        return m;
      }
      // 3) それも無ければ未消費の先頭
      ...
      if (!roundLabel) {
        for (const [k, q] of matchQueues) {
          if (k.startsWith(eventId + '|')) {
            if (q.some(m => m.id != null && !consumed.has(m.id))) return k;
          }
        }
      }
      return null;
    ...
    for (const item of importedSchedule) {
      ...
      // 対応するキューから次の試合を消費（ドロー記載時刻と一致する試合を優先）
      const qKey = findMatchKey(matchedEventId, item.roundLabel);
      if (!qKey) continue;
      const picked = takeFromQueue(qKey, item.startTime);
      if (!picked) continue;
    ...
    for (const [, queue] of matchQueues) {
      for (const m of queue) {
        if (m.id != null && !consumed.has(m.id)) remaining.push(m);
      }
    }
  ...
  const handleConfirmEvent = useCallback(async (eventId: string, skipConfirm = false, skipReorder = false) => {
    // 取込済みドローの開始時刻がタイムゾーンぶんずれている場合は先に補正する。
    // （旧バージョンで取り込んだ 9:00 が 0:00 として保存されているケース。
    //   補正前に対戦表を作ると 0:00 が試合の予定時刻として書き込まれてしまう）
    const tidForRepair = useAppStore.getState().currentTournamentId;
    if (tidForRepair) await repairShiftedDrawTimes(tidForRepair);
    ...
    try {
    // ドローの内容から対戦表を組み直す（入力済みスコアは可能な限り引き継ぐ）
    await rebuildEventMatches(eventId);
  ...
  }, [events, maybeRegenerateSchedule, recalculateGlobalMatchOrder, heatRulePattern]);
  ...
  // ===== Event Section =====
  // === あたり修正の操作 ===
  const startDrawEdit = useCallback((eventId: string) => {
    const draw = drawMap.get(eventId);
    if (!draw) return;
    setDrawEdit({ eventId, slots: draw.slots.slice().sort((a, b) => a.position - b.position).map(s => ({ ...s })) });
    setDrawEditHistory([]);
    setEditSelectedPos(null);
  }, [drawMap]);

  const cancelDrawEdit = useCallback(() => {
    setDrawEdit(null);
    setDrawEditHistory([]);
    setEditSelectedPos(null);
  }, []);

  const pushDrawEdit = useCallback((next: Draw['slots']) => {
    setDrawEdit(cur => {
      if (!cur) return cur;
      setDrawEditHistory(h => [...h, cur.slots]);
      return { ...cur, slots: next };
    });
  }, []);

  const undoDrawEdit = useCallback(() => {
    if (drawEditHistory.length === 0) return;
    const prev = drawEditHistory[drawEditHistory.length - 1];
    setDrawEdit(cur => (cur ? { ...cur, slots: prev } : cur));
    setDrawEditHistory(h => h.slice(0, -1));
    setEditSelectedPos(null);
  }, [drawEditHistory]);

  /** 枠のタップ: 1つ目で選択、2つ目で入れ替え */
  const handleEditSlotTap = useCallback((position: number) => {
    if (!drawEdit) return;
    if (editSelectedPos == null) { setEditSelectedPos(position); return; }
    if (editSelectedPos === position) { setEditSelectedPos(null); return; }
    pushDrawEdit(swapSlotContents(drawEdit.slots, editSelectedPos, position));
    setEditSelectedPos(null);
  ...
  const saveDrawEdit = useCallback(async () => {
    if (!drawEdit) return;
    const { eventId, slots: nextSlots } = drawEdit;
    const draw = drawMap.get(eventId);
    if (!draw?.id) return;

    // 対戦カードが変わって結果が消える試合を先に知らせる
    const existing = allMatches.filter(m => m.eventId === eventId);
    if (existing.length > 0) {
      const draftDraw = { ...draw, slots: nextSlots };
      const isLeague = await isLeagueEvent(eventId, draftDraw);
      const nextMatches = await buildMatchesFromDraw(eventId, draftDraw, isLeague);
      const lost = findResetMatches(nextMatches, existing);
      if (lost.length > 0) {
        const names = lost.slice(0, 5).map(m => `・${m.player1Name} vs ${m.player2Name}（${m.score || '結果あり'}）`).join('\n');
        const more = lost.length > 5 ? `\n…ほか${lost.length - 5}試合` : '';
        const ok = await requestConfirm({
          title: 'あたりの修正',
          message: `対戦の組み合わせが変わるため、次の試合の結果は取り消されます。\n\n${names}${more}\n\n保存してよろしいですか？`,
          danger: true,
          confirmLabel: '保存する',
        });
        if (!ok) return;
    ...
    setSavingDrawEdit(true);
    try {
      await db.draws.update(draw.id, { slots: nextSlots, updatedAt: Date.now() });
      // 対戦表があるときだけ組み直す（未確定なら確定時に作られる）
      if (existing.length > 0) await rebuildEventMatches(eventId);
      cancelDrawEdit();
    } catch (e) {
      console.error('[あたり修正] 保存に失敗:', e);
      alert('保存に失敗しました');
    } finally {
      setSavingDrawEdit(false);
    }
  }, [drawEdit, drawMap, allMatches, requestConfirm, cancelDrawEdit]);

  /**
   * あたり修正パネル。
   * 「4枠ずつのまとまり（＝2回戦で当たる相手が決まる単位）」で区切って並べ、
   * 手書きのドロー表と同じ形にできるようにする。
   */
  const renderDrawEditor = (eventId: string, slots: CheckInSlot[]) => {
    if (!drawEdit || drawEdit.eventId !== eventId) return null;
    const selected = editSelectedPos != null
      ? drawEdit.slots.find(s => s.position === editSelectedPos)
      : undefined;
    const selectedIsEmpty = !!selected && isEmptySlot(selected);
    const byPos = new Map(slots.map(s => [s.drawPosition, s]));

    /** 4枠ごと（2回戦のまとまり）に区切ったブロック */
    const blocks: number[][] = [];
    ...
    const slotButton = (pos: number) => {
      ...
      const raw = drawEdit.slots.find(x => x.position === pos);
      const empty = !raw || isEmptySlot(raw);
      const name = empty
        ? '空き'
        : [s?.playerName, s?.partnerName].filter(Boolean).join(' / ') || '(不明)';
      const isSel = editSelectedPos === pos;
      return (
        <button
          key={pos}
          onClick={() => handleEditSlotTap(pos)}
        ...
        >
          <span className={`w-6 text-[10px] font-mono text-center shrink-0 ${isSel ? 'text-white/80' : 'text-gray-400'}`}>{pos}</span>
          <span className={`flex-1 min-w-0 truncate text-[12px] font-bold ${empty && !isSel ? 'font-normal' : ''}`}>{name}</span>
          {!empty && s?.affiliation && (
            <span className={`text-[10px] truncate max-w-[38%] ${isSel ? 'text-white/80' : 'text-gray-400'}`}>{s.affiliation}</span>
          )}
        </button>
      );
    };
    ...
    return (
      <div className="border-b border-primary-200 bg-primary-50/60 px-3 py-2.5">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mb-2">
          ...
          <span className="text-[11px] text-gray-600 flex-1 min-w-[160px]">
            {editSelectedPos == null
              ? '枠をタップして選ぶと、入れ替え・ずらしができます'
              : `#${editSelectedPos} を選択中 — 入れ替え先をタップ、または右のボタンでずらす`}
          </span>
          {editSelectedPos != null && (
            <>
          ...
          <button onClick={undoDrawEdit} disabled={savingDrawEdit || drawEditHistory.length === 0}
            className="flex items-center gap-1 text-[11px] font-bold text-gray-600 border border-gray-300 bg-white rounded-full px-2 py-1 hover:bg-gray-100 disabled:opacity-40">
            <RotateCcw className="w-3.5 h-3.5" />1つ戻す
          </button>
          <button onClick={cancelDrawEdit} disabled={savingDrawEdit}
            className="text-[11px] font-bold text-gray-600 border border-gray-300 bg-white rounded-full px-2 py-1 hover:bg-gray-100 disabled:opacity-40">
            やめる
          </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
          {blockColumn(blocks.slice(0, half))}
          {blockColumn(blocks.slice(half))}
        </div>
      </div>
    );
  };
  ...
  const renderEventSection = (eventId: string, eventName: string, forceShow = false) => {
    ...
    return (
      <div key={eventId} className={`rounded-xl shadow-sm border overflow-x-auto transition-all ${isConfirmedEvent ? 'bg-gray-100 border-gray-300' : 'bg-white border-border-main'}`}
        ...
        <div className={`px-3 sm:px-4 py-2.5 sm:py-3 border-b flex items-center justify-between sticky top-0 z-10 ${isConfirmedEvent ? 'bg-gray-200 border-gray-300' : 'bg-primary-50 border-border-main'}`}>
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-xs">
            ...
            {drawMap.has(eventId) && (
              <button onClick={(e) => {
                  e.stopPropagation();
                  if (drawEdit?.eventId === eventId) cancelDrawEdit(); else startDrawEdit(eventId);
                }}
                className={`flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-semibold rounded-lg transition-all min-h-[32px] ${
                  drawEdit?.eventId === eventId
                    ? 'text-white bg-primary-600 border border-primary-700'
                    : 'text-gray-600 bg-white border border-gray-200 hover:bg-gray-50'
                }`}
                title="ドロー表とあたりが違う場合に、枠を並べ替えて直します">
                <Shuffle className="w-3.5 h-3.5" />あたり修正
              </button>
            )}
            {!isConfirmedEvent && (
        ...
        {!isCollapsed && renderDrawEditor(eventId, slots)}
