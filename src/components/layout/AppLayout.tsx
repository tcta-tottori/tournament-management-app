...
// 金の微粒子 — 空気中に漂う極小の金色パーティクル
// type: 0=極微粒子(0.5-0.8px), 1=微粒子(0.8-1.2px), 2=小粒子(1.2-1.8px, まれにキラッ)
const GOLD_DUST_PARTICLES: { x: number; y: number; s: number; o: number; t: number }[] = [];

// 極微粒子 (type 0) — 大量に均等散布、ほぼ見えないが密度を出す
for (let i = 0; i < 60; i++) {
  GOLD_DUST_PARTICLES.push({
    x: (i * 1.67 + ((i * 7 + 3) % 11) * 0.3) % 100,
    y: ((i * 13 + 5) % 59) + 1,
    s: 0.5 + ((i * 3) % 4) * 0.1,
    o: 0.15 + ((i * 7) % 5) * 0.04,
    t: 0,
...
// 微粒子 (type 1) — 中密度、ふわっと浮遊する
for (let i = 0; i < 35; i++) {
  GOLD_DUST_PARTICLES.push({
    x: (i * 2.86 + ((i * 11 + 7) % 13) * 0.5) % 100,
    y: ((i * 17 + 3) % 55) + 3,
    s: 0.8 + ((i * 5) % 5) * 0.1,
    o: 0.18 + ((i * 3) % 6) * 0.04,
    t: 1,
...
// 小粒子 (type 2) — 少数、たまにキラッと光るアクセント
for (let i = 0; i < 12; i++) {
  GOLD_DUST_PARTICLES.push({
    x: (i * 8.3 + ((i * 5 + 2) % 7) * 1.5) % 100,
    y: ((i * 19 + 7) % 50) + 5,
    s: 1.2 + ((i * 3) % 4) * 0.2,
    o: 0.2 + ((i * 7) % 4) * 0.05,
    t: 2,
